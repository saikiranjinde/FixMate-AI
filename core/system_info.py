...
def _cpu_generation_series(name: str, vendor: str) -> tuple[str, str]:
    ...
    if "INTEL" in upper:
        m = re.search(r"CORE(?:\([^)]*\))?\s+(I[3579])[- ]([0-9]{3,5}[A-Z]{0,3})", upper)
        if m:
            ...
            if digits:
                ...
                try:
                    ...
                    generation = f"{gen}th Gen" if gen not in (11, 12, 13, 14) else f"{gen}th Gen"
                except ValueError:
                    pass
        else:
    ...
    if "AMD" in upper or "RYZEN" in upper:
        ...
        if m:
            ...
            if digits:
                ...
                try:
                    # Ryzen 7000/8000 families are commonly identified by the first digit(s).
                    generation = f"{int(first[:2])}00 Series"
                except ValueError:
                    pass
...
def get_system_info() -> dict[str, Any]:
    ...
    info = {
        ...
        "cpu_series": series,
        "cpu_base_clock": _format_clock(base_clock),
